"""CEF08 (2x polygon width, no sidebar): keep logic; hide Streamlit sidebar visually."""
...
SD_BASE = 4.0              # SD→px の基礎
POLY_WIDTH_SCALE = 2.0     # 横幅 2倍
    ...
    <script>
      ...
      (function(){
        ...
        function placeInitMarkersOnce(){
          ...
          markers.forEach(m=>{
            if (m.dataset.initPlaced === "1"){
              setPosition(m, float(m.dataset.left||"100"), float(m.dataset.top||"100"));
              return;
            }
        ...
        function updateLayout(){
          ...
          initPlanes(); updatePlanes(); updateAngleStack();
...
def main():
    ...
    st.markdown(
        ...
        <style>
          ...
          /* メイン余白を少し詰める（任意） */
          section.main > div { padding-left: 0.5rem; padding-right: 0.5rem; }
        </style>
        """,
        unsafe_allow_html=True,
    )

    # base 側の UI/ロジックはそのまま利用
