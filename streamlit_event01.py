# CEF24.py — single-canvas, no sidebar, angle-stack scales (plan ②)
#            + consolidated coordinates panel under angles
#            + per-marker floating coord tags removed
...
def render_ceph_component(image_data_url: str, marker_size: int, show_labels: bool, point_state: dict):
    ...
    <style>
      ...
      #angle-stack{
        ...
        background:rgba(15,23,42,.80);color:#f8fafc;
      ...
      /* 座標パネル（角度の下にまとめる） */
      #coords-panel{margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,.25);}
      #coords-title{font-size:12px;font-weight:700;opacity:.9;margin-bottom:4px;}
      #coords-list{max-height:180px;overflow:auto;padding-right:4px;}
      .coord-item{display:flex;justify-content:space-between;font:11px/1.2 monospace;opacity:.95;padding:1px 0;}
    ...
    <div class="ceph-wrapper">
      ...
      <div id="angle-stack">
        __ANGLE_ROWS_HTML__
        <div id="coords-panel">
          <div id="coords-title">Points (px)</div>
    ...
    <script>
      ...
      (function(){
        ...
        function refreshCoordsPanel(){
          // 表示順は payload.points の宣言順を優先（なければ markers 配列）
          const ids = (payload.points||[]).map(p=>p.id);
          const ordered = ids.length
            ? ids.filter(id => markerById[id]).map(id => markerById[id])
            : markers;
          ...
          ordered.forEach(m=>{
            ...
            row.className = "coord-item";
            row.innerHTML = `<span>${id}</span><span>(${x}, ${y})</span>`;
        ...
        function createMarker(pt){
          ...
          m.addEventListener("pointermove",(ev)=>{
            ...
            updatePlanes(); updateAngleStack(); redrawPolygonAndDots(); refreshCoordsPanel();
          });
          const finish=()=>{
            if(activeMarker!==m) return;
        ...
        function updateLayout(){
          ...
          syncAngleStackScale();
          placeInitMarkersOnce(); initPlanes(); updatePlanes(); updateAngleStack(); redrawPolygonAndDots(); refreshCoordsPanel();
        }
...
# ── 最小UIの main を自前で用意（CEF03.main() は呼ばない） ──
def slim_main() -> None:
